...
// so travel runs right-to-left across the graphic. Both routes finish by turning
// north up the Roaring Fork valley (Carbondale/VPAKG) to Aspen.
const SOUTH_POINTS = [
  ...
  { x: 115, y: 96, label: 'GUC', type: 'airport' },
  { x: 75, y: 96, label: 'MTJ', type: 'airport' },
  { x: 40, y: 90, label: 'VPAKG', type: 'airport' },
  { x: 20, y: 92, label: 'ASPEN', type: 'end', anchor: 'middle' },
]
...
const NORTH_POINTS = [
  ...
  { x: 80, y: 90, label: 'VPAKG', type: 'airport' },
  { x: 20, y: 92, label: 'ASPEN', type: 'end', anchor: 'middle' },
]
...
export default function RouteMap({ route }) {
  ...
  return (
    <svg
      viewBox="0 0 460 140"
      role="img"
      aria-label={`Stylized elevation profile of the ${route} route from the Front Range to Aspen, oriented north-up`}
