...
// so travel runs right-to-left across the graphic. Both routes continue south
// past Montrose, over the Cones VOR (ETL), into Telluride.
const SOUTH_POINTS = [
  ...
  { x: 105, y: 96, label: 'GUC', type: 'airport' },
  { x: 65, y: 96, label: 'MTJ', type: 'airport' },
  { x: 35, y: 90, label: 'ETL', type: 'airport' },
  { x: 20, y: 92, label: 'TELLURIDE', type: 'end', anchor: 'middle' },
]
...
export default function RouteMap({ route }) {
  ...
  return (
    <svg
      viewBox="0 0 460 140"
      role="img"
      aria-label={`Stylized elevation profile of the ${route} route from the Front Range to Telluride, oriented north-up`}
